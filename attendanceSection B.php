...
<body>
    ...
    <div class="container my-4">
        ...
        $sql_a = "SELECT fname, lname, roll FROM student_info WHERE section='B' ORDER BY roll";
        ...
        <div class="container my-4"><br>
            <h3>Section - B</h3>
            <p align="left">
                <a href="trackB.php"><button class="btn btn-warning">View Attendance</button></a>
            </p>
            <form method="POST">
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $date = date('d-m-y');
        $query = "SELECT DISTINCT date FROM attendanceb";
        ...
        if (!$bool) {
            foreach ($att as $key => $value) {
                if ($value == "Present") {
                    $query = "INSERT INTO attendanceb (username, status, roll, date) VALUES ('$username', 'Present', '$key', '$date')";
                    $insertResult = $conn->query($query);
                } else {
                    $query = "INSERT INTO attendanceb (username, status, roll, date) VALUES ('$username', 'Absent', '$key', '$date')";
                    $insertResult = $conn->query($query);
                }
            }
            if ($insertResult) {
